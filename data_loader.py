...
class DataLoader():
    def __init__(self, data_dir='DIV2K/DIV2K_train_HR',
        ...
        self.map_parellel_num = map_parallel_num
    ...
    def gen_tfrecords(self, save_dir='DIV2K/tfrecords', tfrecord_num=10):
        ...
        for i in range(file_num):
            fs.append(tf.python_io_TFRecordWriter(os.path.join(save_dir, 'data%d.tfrecords' % i)))
        ...
        print('example number: %d' % sampel_num)
    ...
    def _parse_one_example(self, example):
        ...
        ## 注意，这里的gt都是tensor形式的，所以我们也要使用tf的运算方式
    ...
    #每一个example指包含一个64p的patch
    def read_tfrecord(self, save_dir='DIV2K/tfrecords'):
        fs_paths = sorted(glob.glob(os.path.join(save_dir, '*.tfrecord')))
        if len(fs_paths) == 0:
    ...
    def read_pngs(self):
        dataset = tf.data.Dataset.from_generator(self.gen_generator, (tf.float32, tf.float32, tf.float32))
